...
서버 개발자를 위한 Claude Code 설정. C/C++, Go, Rust, C#, Python 지원. 최신 Claude 모델 대응. macOS / Linux / Windows 동작. 규칙은 `CLAUDE.md`에 인라인되어 상시 적용된다.
...
`CLAUDE.md`는 `~/.claude`에 복사된다(Claude Code 글로벌 지시 파일). 배포는 이 복사 외에 두 가지를 자동 머지한다. 둘 다 키 단위 유니온 머지 — 기존 항목 보존, 쓰기 전 `.bak`, 타겟이 깨진 JSON이면 중단:
...
### Rules (5개 — CLAUDE.md에 인라인)

규칙은 별도 `rules/` 디렉토리가 아니라 `CLAUDE.md`의 "규칙" 섹션에 전문 인라인된다. `CLAUDE.md`는 상시 로드되는 단일 파일이라, 조건부(`paths:`) 로딩 대신 한곳에 통합해 일관되게 적용한다.
...
테스트 기준은 상시 로드에서 빼 `references/testing.md`(온디맨드)로 둔다. 코딩 스타일은 `CLAUDE.md`에 인라인되어 상시 포함된다 — 컨텍스트 비용과 일관성의 트레이드오프다(규칙 전문 합산 약 20 KiB).
...
- 규칙: `CLAUDE.md`의 "규칙" 섹션에서 직접 수정한다. 규칙 전문은 인라인 유지한다 — 1M 컨텍스트(Opus 4.x·Sonnet 4.6, 200K 초과 프리미엄 없음)와 프롬프트 캐시리드 덕분에 분리할 용량·비용 명분이 없고, 핵심 행동 규칙은 항상 적용돼야 하므로 조건부 로딩은 강제를 깨뜨린다. 상시 로드가 길어지면 개별 지시 준수율이 떨어질 수 있으나, 이는 `references/`로 분리해 해결할 문제가 아니라 핵심 규칙을 상단에 두고 저빈도 참조 항목(체크리스트 등)을 뒤로 미루는 순서로 다룬다. `references/`는 테스트 기준처럼 가끔만 필요한 자료에만 쓴다.
