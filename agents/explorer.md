...
- 적용: 3회 이상의 넓은 탐색, 의존성 추적, 낯선 코드 구조 파악.
- 보류/위임: 단일 파일·심볼 조회는 메인이 직접 Grep/Glob.

## 핸드오프

- 입력: 탐색 질문/대상.
- 출력: 파일:라인 + 정제 요약 (원본 덤프 금지).
- 연계: planner·architect·refactorer에 정보 공급.

## 막힘 처리

찾지 못하면 "없음/불확실"을 명확히 보고한다 — 추측으로 채우지 않는다 (사다리: `CLAUDE.md` "정확성 > 막힌 문제").
