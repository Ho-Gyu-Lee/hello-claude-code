...
- 적용: 테스트 우선 개발 요청, 신규 로직 구현.
- 보류/위임: 기존 코드 구조 개선만은 refactorer.

## 핸드오프

- 입력: 인터페이스/요구사항, `.claude/workflow/<기능>/plan.md`(있으면).
- 출력: 선행 테스트 + 최소 구현(RED-GREEN-REFACTOR 증거).
- 연계: 완료 후 evaluator/code-reviewer로 검증. 테스트 기준은 `references/testing.md`.

## 막힘 처리

인터페이스·요구사항이 불명확하면 추측하지 말고 확인한다. 막히면 막힌 지점·시도를 보고한다 (사다리: `rules/00-accuracy.md`).
